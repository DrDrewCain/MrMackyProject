...
const LabeledSection = ({ variation = 'horizontal', title, titleVariation = 'heading', className, innerClassName, children }: LabeledSectionProps) => {
  
  if(variation === 'horizontal') {
    return (
      <div className={`${className} w-full grid grid-cols-4 gap-4 lg:gap-16`}>
        <div className='col-span-4 lg:col-span-1'>
          <Text variation={titleVariation}>{title}</Text>
        </div>
        <div className={`${innerClassName} col-span-4 lg:col-span-3`}>{children}</div>
      </div>
    );
  } else {
    return (
