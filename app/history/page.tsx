...
const timeline = {
  2023: [
    "Professional development workshop (PDW) of the case holds at the Annual Meeting of Academy of Management (Boston, MA)  ",
  ],
  2022: [
    "Winner of the Innovative Teaching Award from the Academy of Management, HR Division (Seattle, WA)",
    "Adapted in HR Metrics & Analytics course (Master's), University of Texas at Arlington (Fall 2022 to present)",
  ],
  2021: [
    "Adapted in HR Metrics & Talent Analytics (Executive Education), Rutgers University",
  ],
  2020: [
    "Revised as online course materials for HR Analytics course (Master's), Rutgers University (Spring 2020 to present)",
  ],
  2019: [
    "The workbook for Mr. Macky's case, Exercises in HR Analytics,  was written ",
    "Adapted in HR Analytics course (Master's), Rutgers University (Fall 2019 to present) ",
  ],
        ...
        <div className="col-span-4 flex flex-col gap-4 lg:col-span-3">
          {Object.keys(timeline)
            ...
            .map((yearStr, index) => {
              ...
              return (
                <div
                  key={`evnts_${year}_index`}
                  className="flex flex-row gap-8"
                >
                  <Text variation="paragraphheading">{year}</Text>
                  <div>
                    {events.map((event, eventIndex) => (
                      <Text key={`${year}_${event}_${eventIndex}`}>
                        {event}
                      </Text>
                    ))}
                  </div>
